...
| Phase progress readout | Every phase the skill names carries `Phase X of Y` with `— N done, M to go` on forward-looking prompts; Y/done/M derived live from the roadmap each run (Y = phase-block count, done = filled `Landed:` lines, M = Y−done−1); build mode states the total at Stage 5 handoff (the first instant a plan exists); single-sourced in `test-pushback.md § Talking to the developer` | Bare `Phase X of Y` (shows tunnel length but not progress through it — the developer explicitly wanted "how far, how much left," which a bare ratio makes them compute); a stored phase counter (goes stale — same failure class as a cached status field, against the completion model's derive-don't-cache principle); **phase-discovery during execution** (the skill inventing phases as it writes tests makes Y drift every run for reasons the developer can't see — a new gap is a *human* edit to the roadmap, deliberate and visible, not silent denominator growth) |
